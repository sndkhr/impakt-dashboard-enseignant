...
function SettRow({ label, sub, children }: { label: string; sub?: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 0', borderBottom: '1px solid rgba(15,15,15,0.04)' }}>
      <div style={{ flex: 1, fontSize: 12, fontWeight: 500, color: 'var(--premium-text-2)' }}>
        {label}
        {sub && <small style={{ display: 'block', fontSize: 10, fontWeight: 400, color: 'var(--premium-text-4)', marginTop: 1 }}>{sub}</small>}
      </div>
      {children}
    </div>
  );
...
const inputStyle: React.CSSProperties = {
  ...
  fontFamily: 'inherit', fontSize: 12, color: 'var(--premium-text)', outline: 'none',
};
...
function ConnItem({ date, time, browser, active }: { date: string; time: string; browser: string; active?: boolean }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid rgba(15,15,15,0.04)', fontSize: 11, color: 'var(--premium-text-2)' }}>
    ...
    <div className="fi" style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 14, alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <ProfCard>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--premium-text)', marginBottom: 12 }}>Mon profil</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--premium-text)' }}>Sophie Brunet</div>
                <div style={{ fontSize: 11, color: 'var(--premium-text-4)' }}>Conseillère en insertion</div>
                <div style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}>Modifier la photo</div>
              </div>
            </div>
            <SettRow label="Nom"><input style={inputStyle} defaultValue="Brunet" /></SettRow>
            <SettRow label="Prénom"><input style={inputStyle} defaultValue="Sophie" /></SettRow>
            <SettRow label="Email"><input style={inputStyle} type="email" defaultValue="s.brunet@missionlocale-bobigny.fr" /></SettRow>
            <SettRow label="Téléphone"><input style={inputStyle} defaultValue="01 48 30 22 15" /></SettRow>
            <SettRow label="Structure">
              <select style={selectStyle} defaultValue="Mission Locale de Bobigny">
                <option>Mission Locale de Bobigny</option>
                <option>France Travail Aubervilliers</option>
                <option>Mission Locale de Saint-Denis</option>
                <option>CCAS Clichy-sous-Bois</option>
              </select>
            </SettRow>
          ...
          <ProfCard>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--premium-text)', marginBottom: 12 }}>Notifications</div>
            <SettRow label="Nouveau bénéficiaire inscrit" sub="Notification à chaque inscription"><Toggle defaultOn /></SettRow>
            <SettRow label="Test IMPAKT terminé" sub="Quand un jeune termine son évaluation"><Toggle defaultOn /></SettRow>
            <SettRow label="Rappel RDV à venir" sub="Notification avant chaque rendez-vous"><Toggle defaultOn /></SettRow>
            <SettRow label="Bénéficiaire inactif" sub="Alerte quand un jeune ne se connecte plus"><Toggle defaultOn /></SettRow>
            <SettRow label="Métier/formation validé" sub="Quand un jeune like un métier ou une formation"><Toggle /></SettRow>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <ProfCard>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--premium-text)', marginBottom: 12 }}>Préférences d&apos;accompagnement</div>
            ...
            <SettRow label="Capacité max de suivi" sub="Bénéficiaires simultanés">
          ...
          <ProfCard>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--premium-text)', marginBottom: 12 }}>Sécurité</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <ProfCard style={{ textAlign: 'center', padding: '20px 16px' }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--premium-text-4)', marginTop: 2 }}>Orientation personnalisée</div>
        </ProfCard>
        ...
        <ProfCard title="Informations">
          {[
            ...
            { label: 'Licence', value: 'Mission Locale 93' },
            ...
            <div key={i} style={{ padding: '8px 0', borderBottom: i < arr.length - 1 ? '1px solid #f5f5f5' : 'none' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--premium-text-4)', textTransform: 'uppercase', letterSpacing: '.3px' }}>{info.label}</div>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--premium-text)', marginTop: 2 }}>{info.value}</div>
            </div>
          ))}
        </ProfCard>
