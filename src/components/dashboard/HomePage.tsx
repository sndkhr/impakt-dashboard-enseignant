...
import { computeParcoursStatus, formatDateFr } from '@/types';
...
export default function HomePage() {
  ...
  const { openFilters } = useModals();
  const [homeSort, setHomeSort] = useState<'name' | 'date' | 'prog'>('date');
  ...
  const sortedTableData = useMemo(() => {
    return [...tableData].sort((a, b) => {
      if (homeSort === 'date') {
        const aTime = a.lastActive ? new Date(a.lastActive).getTime() : a.inscDate.getTime();
        const bTime = b.lastActive ? new Date(b.lastActive).getTime() : b.inscDate.getTime();
        return bTime - aTime;
      }
  ...
  // Upcoming RDV items
  const completedUsers = sortedUsers.filter(u => u.quizCompleted).slice(0, 6);
          ...
          <div>
            ...
            <div className="fi" style={{ background: 'var(--white)', border: '1px solid var(--border)', borderTop: 'none', borderRadius: '0 0 14px 14px', overflow: 'hidden', animationDelay: '.06s' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Nom', 'Prénom', 'Inscription', 'Dernière activité', 'Progrès', ''].map(h => (
                ...
                <tbody>
                  {sortedTableData.map((u, i) => {
                      ...
                      <tr key={i} onClick={() => openProfile(u.uid)} style={{ cursor: 'pointer' }}>
                        <td style={{ padding: '10px 14px', fontSize: '12.5px', borderBottom: '1px solid #f5f5f5', fontWeight: 600, color: 'var(--text-900)' }}>
                          {u.nom || '—'}
                        </td>
                        ...
                        <td style={{ padding: '10px 14px', fontSize: '11px', color: 'var(--text-400)', borderBottom: '1px solid #f5f5f5' }}>
                          {u.lastActive ? new Date(u.lastActive).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }) + ' à ' + String(new Date(u.lastActive).getHours()).padStart(2, '0') + 'h' + String(new Date(u.lastActive).getMinutes()).padStart(2, '0') : '—'}
                        </td>
                        <td style={{ padding: '10px 14px', borderBottom: '1px solid #f5f5f5' }}>
                          <ProgressBar value={u.prog} />
                        </td>
                        <td style={{ padding: '10px 14px', borderBottom: '1px solid #f5f5f5' }}>
                          <button onClick={(e) => { e.stopPropagation(); openProfile(u.uid); }} style={{ fontSize: 11, padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--white)', fontFamily: 'inherit', fontWeight: 500, color: 'var(--text-700)', cursor: 'pointer' }}>Voir ›</button>
                        </td>
                      </tr>
                    );
                  })}
